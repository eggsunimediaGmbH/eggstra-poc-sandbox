...
export default function decorate(block) {
    ...
    const [
        galleryHeadline, galleryDescription
    ...
    [...block.children].forEach((item: HTMLElement) => {
        // manipulate gallery items
        if (!hasClass(item)) {
            item.className = `${blockName}__carousel-item`;

            const picdiv = item.children[0] as HTMLElement;
            const textdiv = item.children[1] as HTMLElement;
            const hrefdiv = item.children[2] as HTMLElement;
    ...
    galleryCarousel.addEventListener('wheel', (event: WheelEvent) => {
