import {debounce, getDeviceType, changeHeightAndPositionInEditor} from "../../scripts/funcs/storageUtils";
import {isVideoLink, createVideo} from "../../scripts/video-support";
import {observeElement} from "../../scripts/aem";
import {isSafariUserAgent} from "../../scripts/funcs/editorUtils";


const blockName = 'hero';

export default async function decorate(block: HTMLElement): Promise<void> {
    const [videoContainer, firstHeadlineContainer, secondHeadlineContainer, heroDescriptionContainer, bannerContainer] = Array.from(block.children) as HTMLElement[];

    const heroVideoContainer = videoContainer as HTMLElement;
    heroVideoContainer.classList.add(`${blockName}__video`);

    let heroContainer = videoContainer.querySelector('div') as HTMLElement | null;
    heroContainer?.classList.add(`${blockName}__video-container`);
    ...
    const firstHeadline = firstHeadlineContainer.querySelector('div') as HTMLElement | null;
    firstHeadline?.classList.add(`xl-headline`, `${blockName}__xl-headline--yellow`);

    const secondHeadline = secondHeadlineContainer.querySelector('div') as HTMLElement | null;
    secondHeadline?.classList.add(`${blockName}__intro-headline`);

    const descriptionText = heroDescriptionContainer.querySelector('div') as HTMLElement | null;
    descriptionText?.classList.add(`${blockName}__description`);

    const bannerContainerMobile = bannerContainer as HTMLElement;
    bannerContainerMobile?.classList.add(`${blockName}__banner-container`);

    const bannerTextMobile = bannerContainer.querySelector('div') as HTMLElement | null;
    bannerTextMobile?.classList.add(`${blockName}__banner-text`);
    setupBannerContainer(bannerContainerMobile, true);

    const bannerContainerDesktop: HTMLElement = bannerContainerMobile.cloneNode(true) as HTMLElement;
    ...
    if (isVideo) {
        heroContainer = await createVideo(videoUrl.title.split('/').pop().split('.mp4')[0], heroContainer, `${blockName}__video-element`) as HTMLDivElement;
        videoUrl.remove();
    } else {
...
function moveDefaultContentWrapper(direction:string): void {
...
function setupBannerContainer(bannerContainer: HTMLElement, isMobile: boolean): void {
    ...
    if (paragraphs.length > 0) {
        paragraphs.forEach((p, index) => {
            ...
            if (index === 0) p.classList.add('show');
        });
    }
...
function removeButtonContainerClass(element: HTMLElement): void {
    element.classList.remove('button-container');
}

function matchContainerHeights(): void {
    const leftContainer = document.querySelector('.hero__left-container') as HTMLElement | null;
    const rightContainer = document.querySelector('.hero__right-container') as HTMLElement | null;
    const video = rightContainer?.querySelector('.hero__video') as HTMLElement | null;
    ...
    const leftHeight: number = leftContainer.offsetHeight;
    const rightHeight: number = rightContainer.offsetHeight;
...
function waitForElements(selectors: string[], callback: () => void, timeout: number = 5000): void {
