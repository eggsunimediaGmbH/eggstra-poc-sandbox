...
export default async function decorate(block: HTMLElement) {
    ...
    const backgroundImage = createBackgroundImageWrapper(video1 as HTMLElement, `${blockName}`, videoUrl, isVideo);
...
function handleContent(block: HTMLElement) {
...
function createElement(tag: string, attributes: Record<string, unknown>): HTMLElement {
...
function decorateTitleElement(htmlElement: HTMLElement, hover: boolean, contactCta: HTMLElement) {
...
function decorateTextElement(htmlElement: HTMLElement, hover: boolean, contactCta: HTMLElement) {
    Array.from(htmlElement.children).forEach((child) => {
...
function createPElement(child: ChildNode): HTMLParagraphElement {
    const newPElement = document.createElement('p');
    newPElement.className = (child as HTMLElement).className;
    newPElement.innerHTML = (child as HTMLElement).innerHTML;
    newPElement.classList.add(`${blockName}__content`);

    return newPElement;
}

function setElementActiveOnHover(newPElement: HTMLParagraphElement, hover: boolean, contactCta: HTMLElement) {
    if (contactCta) {
        if (hover) {
