import {setCookie} from "../../scripts/funcs/storageUtils";

const blockName = 'navigation-menu';
let overlay: HTMLElement | null = null;

export default async function attachNavigation(block: HTMLElement): Promise<void> {
    const burgerIcon:Element = block.querySelector('.header__burger-icon');
    const navigationMenu : HTMLElement = await createNavigationMenu();
...
const createGetInTouch = async (): Promise<Node> => {
    const getInTouch: HTMLDivElement = document.createElement('div');
    getInTouch.className = `${blockName}__get-in-touch`;

    const mailElement: HTMLDivElement = document.createElement('div');
    const svgElement : SVGSVGElement = await generateSVG(getIconPath('message', 'header'));
    const getInTouchText: HTMLHeadingElement = document.createElement('h3');
...
const generateSocialIcons = async (): Promise<Node> =>{
    const socialIcons = document.createElement('ul');
    socialIcons.className = `${blockName}__social-icons`;
    const iconsSrc:string[] = ["facebook", "linkedin", "instagram"];

    for (const icon of iconsSrc) {
        const li: HTMLLIElement = document.createElement('li');
        li.className = `social-icon__${icon}`;
        const svgElement: SVGSVGElement = await generateSVG(getIconPath(icon, 'social-icons')) as SVGSVGElement;
        if(svgElement){
...
async function createNavigationMenu(): Promise<HTMLElement> {
...
const topItems = async (): Promise<HTMLElement> => {
    const iconsSrc:string[] = ["light","search", "person", "close"];
    const menuList:HTMLUListElement = document.createElement('ul');
    menuList.className = `${blockName}__top-items`;

    for (const icon of iconsSrc) {
        const li:HTMLLIElement = document.createElement('li');
        li.className = `top-item__${icon}`;
        const svgElement:SVGSVGElement = await generateSVG(getIconPath(icon, 'header'));
        if(svgElement){
            li.appendChild(svgElement);
            if(icon === 'light'){
                const svgDark:SVGSVGElement = await generateSVG(getIconPath("moon", 'header'));
        ...
        if (icon === 'close') {
            li.addEventListener('click', () => {
                const navigationMenu: HTMLElement = document.querySelector(`.${blockName}`) as HTMLElement;
                if (navigationMenu) {
                    navigationMenu.classList.remove('navigation-menu--active');
                    overlay.classList.remove('navigation-menu__overlay--active');
                    document.body.classList.remove('no-scroll');
        ...
        if (icon === 'light') {
            const lightSwitch: HTMLElement = document.querySelector('.top-item__light') as HTMLElement;
            const svgMoonElement: SVGSVGElement = await generateSVG(getIconPath('moon', 'header'));
            if(svgMoonElement && lightSwitch){
                lightSwitch.appendChild(svgMoonElement);
            }
            li.addEventListener('click', (event) => {
                event.stopPropagation();
                const themeIsBlack: boolean = document.body.classList.contains('dark-mode');
                if(themeIsBlack){
...
const attachLanguageDropdown = async (menu:HTMLElement): Promise<void> => {
...
<!--        <div class="top-item__language-dropdown__option">FR</div>-->
    ...
    const imgArrowSvg = await generateSVG(getIconPath('arrowdown', 'header')) as SVGSVGElement;
    if(imgArrowSvg){
...
const getIconPath = (icon:string, folder?: string):string => {
...
const generateSVG = async (path: string): Promise<SVGSVGElement | null> => {
    try {
...
const attachMenuItems = (): HTMLElement => {
